...
void Coap::handler()
{
	...
	if (_ethudp!=NULL) {
		...
		if (size>0) {
			_wifly->getHostIP(bufIp, sizeof(bufIp));
			port = _wifly->getHostPort();
			Serial.print("Receiving from ");
			Serial.print(bufIp);
			Serial.print(":");
			Serial.println(port);
			packet_len = _wifly->gets((char *)_packet_buffer, sizeof(_packet_buffer));
			receiver(_packet_buffer, _wifly->atoip(bufIp), port, packet_len);
...
//	_ethudp->endPacket();
	_wifly->sendto(buffer, size, ip, port);
	_wifly->flush();
}
	...
	for (i = 0; i < _retransmit_slot_counter; i++) {
		...
		if (_retransmit[i].mid != -1) {
			...
			if (_retransmit[i].timestamp <= millis() - 60) {
				...
				udp_send(_retransmit[i].ip, _retransmit[i].port,_retransmit[i].packet, _retransmit[i].size);
